...
DDS_MESSAGES = os.getenv('DDS_MESSAGES')
sys.path.append(DDS_MESSAGES)
...
class Environment:
    def __init__(self):
        with open(f"{DIR_PATH}/bin/python/pydds/topics.json", 'r') as json_file:
    ...
    def find_messages(self):
        ...
        for i in range(4):
            cur_dir=os.getcwd()
            if DDS_MESSAGES:
            	dirs=glob.glob(DDS_MESSAGES + "/**/*pb2.py", recursive=True)
            elif cur_dir == "/":
