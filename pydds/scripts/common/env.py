...
RECORD_ROOT_PATH = os.getenv('RECORD_ROOT_PATH')

if RECORD_ROOT_PATH != None:
    sys.path.append(RECORD_ROOT_PATH + '/../bin/python/messages')
    print("MSG_ROOT_PATH: " + RECORD_ROOT_PATH + '/../bin/python/messages')
def ddprint(args):
...
class Environment:
    ...
    def import_proto_type(self,path):
        ...
        ddprint("### module_string: " + module_string)
    ...
    def find_messages(self):
        ...
        for i in range(4):
            cur_dir=os.getcwd()
            if cur_dir == "/":
                dirs=glob.glob("/home/nio/Videos/20230331T190552.SOC1/bin/python/messages/**/*pb2.py", recursive=True)
            elif RECORD_ROOT_PATH != None:
                dirs=glob.glob(f"{RECORD_ROOT_PATH}/../bin/python/messages/**/*pb2.py", recursive=True)
            else:
