...
parser.add_argument("-planning_dir", "--planning_file_directory", help="visualize planning output file")
parser.add_argument("-control_dir", "--control_file_directory", help="visualize control output file")
...
dict_pb_msg[dict_topic['control_data']] = []
...
dict_aligned_index_of_dt[dict_topic['control_data']] = []

cnt = 0
plan_file_num = len(os.listdir(args.planning_file_directory))
for i in range(plan_file_num):
...
control_file_num = len(os.listdir(args.control_file_directory))
for i in range(control_file_num):
    data_file = "../data/scene_test/follow_scene/control_data/control_data_counter_" + str(i) + ".txt"
    print(info() + 'control data json file: ' + data_file)
    control_data_str = ""
    with open(data_file, "r") as file_obj:
        control_data_str = file_obj.read()
        control_data = control_data_pb2.ControlData()
        json_format.Parse(control_data_str, control_data)
        control_pub_t = control_data.publish_ptp_ts
        dict_pb_msg[dict_topic['control_data']].append([int(control_pub_t), control_data])

record_parser = RecorcParser(dict_topic, 0, 1e18)
record_parser.dict_pb_msg = dict_pb_msg
...
control_data = msg_parser.ControlData(dict_pb_msg[dict_topic['control_data']])
...
callback = CustomJS(args=dict(
    ...
    const ctrl_index = aligned_index[channel['control_data']][step][0];
